...
./lovecraft-to-paras.sh | sed "s/$givenNamesGrep $surnamesGrep/{\$\$protagonistName}/g;s/$surnamesGrep/{\$\$protagonistLName}/g;s/$cultsGrep/{\$\$cult}/g;s/$textsGrep/{\$\$arcaneText}/g" | sed 's/,/\\,/g;s/"/\\"/g;s/\$\([0-9]\)/\1/g' | awk 'BEGIN{ORS=","} {if(!i) { print "\nlovecraftPara" (++j) ":=" ; i=100 }; print $0; i-- } END { print "\nlovecraftPara:=" ; while(j) { print "$lovecraftPara" j ; j-- ;} print "\n"}' | sed 's/=,/=/;s/,$//'
