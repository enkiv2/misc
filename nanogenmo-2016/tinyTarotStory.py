...
# tarot_interpretations.json from dariusk/corpora
tarot={
...
ftTense={
	"Watch":["Watched", "Watches", "Will watch"],
	"Prepare":["Prepared", "Prepares", "Will prepare"],
	"comes":["came", "comes", "will come"],
	"may play":["played", "plays", "will play"],
	"current":["past", "current", "future"],
	"arrives":["arrived", "arrives", "will arrive"],
	"may surface":["surfaced", "surfaces", "will surface"],
	"knows":["knew", "knows", "will know"],
	"is coming":["came", "comes", "will come"],
	"will give":["gave", "gives", "will give"],
	"Expect to be caught":["You were caught", "You are caught", "You will be caught"],
	"will come":["came", "comes", "will come"],
	"blossoms":["blossomed", "blossoms", "will blossom"],
	"becomes":["became", "becomes", "will become"],
	"will be":["was", "is", "will be"],
	"will seek":["sought", "seeks", "will seek"],
	"begins":["began", "begins", "will begin"],
	"departs":["departed", "departs", "will depart"],
	"proves":["proved", "proves", "will prove"],
	"will be rendered soon":["was rendered", "is rendered", "will be rendered"],
	"is making":["made", "is making", "will make"],
	"is revealed":["was revealed", "is revealed", "will be revealed"],
	"is working":["was working", "is working", "will work"],
	"ends":["ended", "ends", "will end"],
	"Someone's using":["Someone used", "Someone's using", "Someone will use"],
	"Cancel":["Canceled", "Cancel", "Cancel"],
	"reverse":["reversed", "reverse", "reverse"],
	"wants":["wanted", "wants", "will want"],
	"needs":["needed", "needs", "will need"],
	"is about to":["was about to", "is about to", "will"],
	"Everything's coming":["Everything came", "Everything is coming", "Everything will come"]
}
meanTense=["ed", "s", "s"]

tenses={"past":0, "present":1, "future":2}
tenses_r=["past", "present", "future"]

def ft2tense(phrase, tense):
	t=tenses[tense]
	for key in ftTense.keys():
		phrase=phrase.replace(key, ftTense[key][t])
	phrase=phrase.replace("This card represents ", "").replace(" who ", " ")
	return phrase
def mean2tense(phrase, tense):
	return phrase.replace("ing ", meanTense[tenses[tense]]+" ", 1).replace("Beed ", "Was ", 1).replace("Bes ", "Is ", 1).replace("Be ", "Will be ", 1).replace("eed ", "ed ", 1)

def threeCardSpread():
	reading=""
	types=["fortune_telling", "meanings"]
	cards=[]
	for i in range(0, 3):
		cards.append(random.choice(tarot["tarot_interpretations"]))
	for i in range(0, 3):
		ch=random.choice(types)
		if(ch=="fortune_telling"):
			reading+=ft2tense(random.choice(cards[i]["fortune_telling"]), tenses_r[i]).lower().replace("your", "her").replace("you", "she").capitalize()+". "
		else:
			reading+=("She "+mean2tense(random.choice(cards[i]["meanings"][random.choice(["light", "shadow"])]), tenses_r[i]).lower().replace("your", "her").replace("you", "she")).capitalize()+". "
	return reading
print(threeCardSpread())
